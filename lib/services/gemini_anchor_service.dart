...
class GeminiAnchorService {
  ...
  Future<List<GeminiAnchorSuggestion>> suggestAnchors({
    ...
    if (maxAnchors != null && cvCandidates.length > maxAnchors) {
      cvCandidates = cvCandidates.sublist(0, maxAnchors);
    }
    ...
    // יעד-אימות ומספר-סבבים — מוגבלים ב[maxAnchors] (מצב מהיר).
    final targetVerified =
        maxAnchors != null ? min(maxAnchors, _targetVerified) : _targetVerified;
    final maxRounds = maxAnchors != null ? 1 : _maxRounds;
    ...
    for (var round = 1; round <= maxRounds; round++) {
      ...
      if (all.where((s) => s.verified == true).length >= targetVerified) {
        break;
      }
    }
    ...
    for (final r in results) {
      ...
      found.add(
        GeminiAnchorSuggestion(
          ...
          confidence: a.confidence,
          basis: a.basis,
          verifyKind: AnchorVerifyKind.vision,
        ),
      );
    }
  ...
  Future<String> _generateOllama(Map<String, dynamic> body) async {
    ...
    try {
      resp = await http
          ...
          // מודל-ראייה מקומי על CPU איטי מאוד עם כמה תמונות (שלב ההתאמה).
